  ...
  describe 'build' do
    ...
    it 'should run a workflow tree and write some results to redis - success' do
      Sidekiq::Testing.disable! do
        ...
        workflow = SidekiqWorkflows.build(workflow_uuid: workflow_uuid, current_attributes: { key: 'value' }) do
        ...
        sleep 10
        expect(Redis.new(url: 'redis://redis_sidekiq_workflow:6379').smembers(workflow_uuid).to_set).must_equal(["WorkerFive - level 2", "WorkerThree - level 2", "WorkerOne - level 1", "WorkerTwo - level 1"].to_set)

        Sidekiq::Batch::Status.new(batch_id).join

        expect(Redis.new(url: 'redis://redis_sidekiq_workflow:6379').smembers(workflow_uuid).size).must_equal(7)
        expect(Redis.new(url: 'redis://redis_sidekiq_workflow:6379').smembers(workflow_uuid)).must_include('success')
        expect(Redis.new(url: 'redis://redis_sidekiq_workflow:6379').smembers(workflow_uuid)).wont_include('death')
      end
    end
    ...
    it 'should run a workflow tree and write some results to redis - death' do
      Sidekiq::Testing.disable! do
        ...
        expect(Redis.new(url: 'redis://redis_sidekiq_workflow:6379').smembers(workflow_uuid).size).must_equal(5)
        expect(Redis.new(url: 'redis://redis_sidekiq_workflow:6379').smembers(workflow_uuid)).wont_include('success')
        expect(Redis.new(url: 'redis://redis_sidekiq_workflow:6379').smembers(workflow_uuid)).must_include('death')
      end
    end
  end
end
